...
if ndims(modes)==4
    ...
    % A.B. : Put the below in as a way to move the code forward but flag a warning, if NaN or Inf occur.    
    out_of_range = isnan(modes) | isinf(modes);
    if any(out_of_range,'all')
        warning('probe modes had inf or NaN values there were reset to one.');
        modes(out_of_range) = 1;
    end
