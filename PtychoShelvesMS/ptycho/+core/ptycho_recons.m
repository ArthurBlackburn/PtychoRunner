...
finishup = utils.onCleanup(@(x) ptycho_exit(x), p);
    ...
    function ptycho_call()
        ...
        if ~p.queue.remote_recons || p.queue.isreplica
            ...
            [p, status] = core.initialize_ptycho(p);
            
             % load known probe, added by Zhen Chen
            if isfield(p, 'known_probe')
                if p.known_probe && isfield(p, 'known_probe_path')
                    prev_d=load(p.known_probe_path);
                    probes0=prev_d.outputs.probe;
                    clear prev_d;
                    disp(strcat('Load probes from: ',p.known_probe_path));
                    disp('only use the first Nmodes and do interpolation if needed!')
                    % check size, probes0 is cell, ignore larger dimensions
                    for i_temp = 1: min([length(probes0),p.probe_modes])
                        if any(   size(probes0{i_temp}(:,:,1,1)) ~= size(p.probes(:,:,1,i_temp)) )
                            p.probes(:,:,1,i_temp)=imresize(probes0{i_temp}(:,:,1,1),size(p.probes(:,:,1,i_temp)),'bilinear');
                        else
                            p.probes(:,:,1,i_temp)=probes0{i_temp}(:,:,1,1);
                        end
                    end
                end
            end

            % load known object, added by Zhen Chen
            if isfield(p, 'known_object')
                if p.known_object && isfield(p, 'known_object_path')
                    prev_d=load(p.known_object_path);
                    object0=prev_d.outputs.object;
                    disp(strcat('Load probes from: ',p.known_probe_path));
                    disp('only use the first Nlayers and do interpolation if needed!')
                    for i_temp=1:length(object0) %min([length(object0),length(p.object) ])
                        if any (  size(object0{i_temp}) ~= size(p.object{1})  )
                            p.object{i_temp}=imresize(object0{i_temp},size(p.object{1}),'bilinear');
                        else
                            p.object{i_temp}=object0{i_temp};
                        end
                    end
                    clear prev_d;
                end
...
%             p.Np_p_presolve=size(p.probes(:,:,1,1));
            ...
            for ieng=1:length(p.engines)
                ...
                % Commented out by ZC to save  memory
