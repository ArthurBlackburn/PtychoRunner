...
if p.model_object
    switch p.model.object_type
        ...
        case 'flat'
            for obnum = 1:p.numobjs 
                p.object{obnum} = exp(1i*(pi/2)*ones([p.object_size(obnum,:) p.object_modes])); 
            end
        case 'flat0'
            for obnum = 1:p.numobjs 
                p.object{obnum} = exp(1i*(0.01)*ones([p.object_size(obnum,:) p.object_modes])); 
            end    
        otherwise
             warning('Invalid case for model.object_type, using default random initial guess')
             for obnum = 1:p.numobjs 
                p.object{obnum} = (1+1i*1e-6*rand([p.object_size(obnum,:) p.object_modes]));
             end            
    end
    ...
    for ii = unique(p.share_object_ID) % avoid loading datasets twice
        ...
        [~,~,ext]=fileparts(p.initial_iterate_object_file{ii});
        switch ext
            case '.hdf5'
                S = io.load_ptycho_recons(p.initial_iterate_object_file{ii});
                object = double(S.object);
            case '.mat'
                S = load(p.initial_iterate_object_file{ii});
                object = S.recon_dat.obj_int{end};
            otherwise
                error(['Unrecognized file type for loading initial object: ' ext])
        end
