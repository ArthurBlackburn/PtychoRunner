...
function [self, param, p] = load_from_p(param, p)
    ...
    %% for output intermediated results, added by YJ
    ...
    self.probe = cell (min(p.probe_modes, size(probes,4)),1); % force to a column cell convenient for multilayer, by Zhen Chen
    ...
    if any(sum(sum(self.diffraction)) / prod(p.asize) < 0.0001) % relax from 0.01 to 0.0001 for low-dose
        error('%i patterns has average photon count < 0.0001', mean(sum(sum(self.diffraction)) / prod(p.asize) < 0.01))
    end
    ...
    if isfield(p, 'delta_z')  % quick fix for Nlayers =1 by Zhen Chen
