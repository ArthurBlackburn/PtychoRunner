...
%     Obj_size_limit = 4096 ; % object size limit in MB, if larger, use CPU instead of GPU by ZC - Mod by AB to speed up
    ...
    for kk = 1:size(self.object,1)
        %       large object with many layers, use CPU to save GPU memory by ZC
      if  numel(self.object{kk,1}) * size(self.object,2)  > Obj_size_limit && par.use_gpu % limit is 100 MB, can be relaxed if more memory is available
