...
function aPsi = modulus_with_background(Psi, background_value, background_shape)
    ...
    % sqrt is very slow ... [Comment from AB: Maybe use quick sqrt hack:
    % expansion + floating pnt trickery?]
    aPsi = sqrt(aPsi2);
...
%                 [nom,denom] = Gfun(@get_background_estimate,modF, aPsi2, mask,  cache.background_weight, cache.background_profile );
                % someone forgot to do a fft_shift. Corrected by Arthur....
                [nom,denom] = Gfun(@get_background_estimate,modF, aPsi2, mask,  fftshift_2D(cache.background_weight), cache.background_profile );
...
%                 self.background = posit(self.background + (par.grouping/self.Npos)*mean(update));
                self.background = max(self.background + (par.grouping/self.Npos)*mean(update),0); 
