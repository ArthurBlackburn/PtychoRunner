...
%                     [pos_update, cache] = gradient_position_solver(self, chi, obj_proj{1},probe{1,layer}, g_ind, iter, cache);
                    % Arthur modified the line above, to reflect the actual
                    % function signature with par included!
                    % Likely someone updated the gradient solver in the
                    % past, but forgot to update all the calls to it, or
                    % did not test it, etc.
                    [pos_update, pos_rotate_upd, probe_scale_upd, cache]  = ...
                                          gradient_position_solver(self, chi, obj_proj{1},probe{1,layer}, g_ind, iter, cache, par);

                    self.modes{1}.probe_positions(g_ind,:)=self.modes{1}.probe_positions(g_ind,:)+pos_update;
                    self.modes{1}.probe_scale_upd(end+1)=self.modes{1}.probe_scale_upd(end)+mean(probe_scale_upd); % added in the same way as applied in lsqml, by Arthur
                    self.modes{1}.probe_rotation_all(g_ind)=self.modes{1}.probe_rotation_all(g_ind)+squeeze(pos_rotate_upd); % added in the same way as applied in lsqml, by Arthur
