...
%
% Citation for this algorithm: at present none, although thanks would be 
% appreciated if you find this script useful. 
%
...
% on the actual object used to write DM3 format files so it is not pratical
% to output to DM3 format.  
...
function [dm3struct] = DM3Import( dm3filename )
    ...
    for index = 1:numel(datatypeloc)
            ...
            if( spectracount == 1 )
                ...
                if( ~ isempty( ydimloc ) ) % This doesn't work either
                    ...
                    if( ~ isempty( zdimloc ) )
                        ...
                        dm3struct.image_data = reshape( readTagData( dataloc(index) ), ydim, xdim );
                    end
                else
        ...
        elseif( datatype(index) == 7 || datatype(index) == 10 || datatype(index) == 11 || datatype(index) == 1 ) % 2-D image
            % last case == 1 added by Arthur 30 sep 2011
            disp( 'Parsing Image' )
            % The tag group 'Dimensions' contains two TagDatas with no names that
            % are the x and y-axis dimensions.  The lack of names mean we need to
            % do some fancy footwork to grab the correct locations.
...
% scales.
    ...
    timestamploc = regexp( metadm3file, 'TimeStamp%%%%' );
    if( ~ isempty( timestamploc ) )
        dm3struct.AcquisitionStartTimeStamp = readTagData( timestamploc(1) );
    end
...
function [typestring] = parseDataType()
    ...
    % STRING  = 18,
    % ARRAY   = 20
    % below cast to float arthur
    if ischar(tagdatatype)
        tagdatatype = str2double(tagdatatype);
    end
%     tagdatatype = double(tagdatatype);
    switch tagdatatype
    case 2
        typestring = 'short';
    case 3
...
function [td] = readDM3Array()
    ...
    if strcmp( tagdatatype, 'DM3struct' )
        ...
        'I do not read arrays of structs'
